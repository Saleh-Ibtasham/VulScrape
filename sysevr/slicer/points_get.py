...
def get_all_sensitiveAPI(db):
    fin = open("./sysevr/slicer/sensitive_func.pkl", 'rb')
...
def deliverSlicePoints(working_directory):
    j = JoernSteps()
    j.connectToDatabase()
    ...
    f = open(slice_point_path + "sensifunc_slice_points.pkl", 'wb')
    pickle.dump(_dict, f, True)
    f.close()
    print(_dict)
    
    _dict = get_all_pointer_use(j)
    f = open(slice_point_path + "pointuse_slice_points.pkl", 'wb')
    pickle.dump(_dict, f, True)
    f.close()
    print(_dict)
    
    _dict = get_all_array_use(j)
    f = open(slice_point_path + "arrayuse_slice_points.pkl", 'wb')
    pickle.dump(_dict, f, True)
    f.close()
    print(_dict)
    
    _dict = get_all_integeroverflow_point(j)
    f = open(slice_point_path + "expr_slice_points.pkl", 'wb')
    pickle.dump(_dict, f, True)
    f.close()

...
#
#     _list.sort(key=lambda x: (str(x[0]), str(x[1])))
#
#
...
def sortedNodesByLoc(list_node):
    ...
    # for x in _list:
    #     if (x[2]['location'] == None):
    #         print("Slice order: ",x[0],x[1],x[2])
    #     else:
    #         print("Slice order: ", x[0], x[1], x[2])
...
def getFuncPDGById(testID, pdg_funcid):
    working_directory = directory + "pdg_db"
    file_dir = os.path.join(working_directory, testID)
...
def getFuncPDGById_noctrl(testID, pdg_funcid):
    ...
    for _dir in os.listdir(working_directory):
        ...
        if testID not in list_testid:
            ...
            file_dir = os.path.join(working_directory, _dir, testID)

...
def deliverePdg(working_directory):
    char_list = ['\\', '/', ':', '*', '?', '\"', '<', '>', '|',' ']
    ...
    for node in all_func_node:
        ...
        path = os.path.join(pdg_db_path, testID)

        store_file_name = node.properties['name']
        for character in char_list:
            store_file_name = store_file_name.replace(character,"_")
        store_file_name = store_file_name + '_' + str(node._id)
        ...
        cfg_path = os.path.join(cfg_db_path, testID, store_file_name)
        for _file in os.listdir(cfg_path):
            if _file == 'dict_if2cfgnode':
                fin = open(os.path.join(cfg_path, _file), 'rb')
